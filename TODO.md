...
> **Last Updated:** December 28, 2025 14:35 UTC
...
| 2 | Enable noUncheckedIndexedAccess | 95% | 12 | Pending |
| 3 | Lint Warning Cleanup (221 warnings) | 92% | 15 | Pending |
| 4 | Edge Function TypeScript Strict Mode | 88% | 14 | Pending |
| 5 | Type Coverage Reporting & Monitoring | 85% | 13 | Pending |
| 6 | Pre-Commit Hook for Type Safety | 90% | 11 | Pending |
| 7 | TypeScript Project References | 82% | 16 | Pending |
| 8 | Additional Strict Flags (noPropertyAccessFromIndexSignature) | 87% | 10 | Pending |
| 9 | Edge Function Integration Tests | 85% | 16 | Pending |
| 10 | Type-Safe Environment Variables | 93% | 12 | Pending |

### Recent Bug Fixes (v1.12.0)
11 bugs fixed with December 2025 research: 1 CRITICAL, 2 HIGH, 7 MEDIUM, 1 LOW

---

## Action 1: Consolidate to "strict": true Mode

**Confidence: 100%** | **Priority: CRITICAL** | **Effort: Low**

### Problem
All individual strict flags are enabled (`noImplicitAny`, `strictNullChecks`, `strictFunctionTypes`, etc.), but the consolidated `"strict": true` flag is still `false`. This creates redundancy and doesn't benefit from future strict flags.

### Research Citations
- [TypeScript Handbook - Compiler Options](https://www.typescriptlang.org/tsconfig#strict) - Official docs explain that `strict: true` enables all strict family flags
- [TypeScript 5.8 Release Notes](https://devblogs.microsoft.com/typescript/announcing-typescript-5-8/) - Latest strict mode enhancements

### Atomic Subtasks

| # | Task | Files (≤5) | Est. |
|---|------|------------|------|
| 1.1 | Remove individual strict flags | `tsconfig.app.json` | 5min |
| 1.2 | Enable `"strict": true` | `tsconfig.app.json` | 2min |
| 1.3 | Update root tsconfig.json | `tsconfig.json` | 2min |
| 1.4 | Run `npm run typecheck` | Terminal | 2min |
| 1.5 | Run `npm run validate` | Terminal | 3min |
| 1.6 | Update comments in tsconfig | `tsconfig.app.json` | 5min |
| 1.7 | Document in CHANGELOG | `CHANGELOG.md` | 5min |
| 1.8 | Update CLAUDE.md tech stack | `CLAUDE.md` | 10min |
| 1.9 | Verify build succeeds | Terminal | 3min |
| 1.10 | Test production build | Terminal | 5min |
...
### Problem
`noUncheckedIndexedAccess: false` allows unchecked array/object access. `array[0]` returns `T` instead of `T | undefined`, causing potential runtime errors.

### Research Citations
- [TypeScript 4.1 Release](https://devblogs.microsoft.com/typescript/announcing-typescript-4-1/#no-unchecked-indexed-access) - Flag introduction
- [TypeScript Best Practices 2025](https://medium.com/@nikhithsomasani/best-practices-for-using-typescript-in-2025) - Recommends enabling this flag

### Atomic Subtasks

| # | Task | Files (≤5) | Est. |
|---|------|------------|------|
| 2.1 | Enable `noUncheckedIndexedAccess` | `tsconfig.app.json` | 5min |
| 2.2 | Run typecheck to identify errors | Terminal | 5min |
| 2.3 | Fix array access in hooks/ | `src/hooks/**/*.ts` | 45min |
| 2.4 | Fix array access in components/ | `src/components/**/*.tsx` | 60min |
| 2.5 | Fix array access in lib/ | `src/lib/**/*.ts` | 30min |
| 2.6 | Add optional chaining where needed | Various files | 30min |
| 2.7 | Add array bounds checks | Various files | 20min |
| 2.8 | Run typecheck again | Terminal | 5min |
| 2.9 | Run full test suite | Terminal | 10min |
| 2.10 | Update build scripts | Terminal | 5min |
| 2.11 | Document in CHANGELOG | `CHANGELOG.md` | 5min |
| 2.12 | Add comments for tricky cases | Various files | 15min |

---

## Action 3: Lint Warning Cleanup (221 warnings)

**Confidence: 92%** | **Priority: HIGH** | **Effort: High**

### Problem
221 ESLint warnings (220 warnings + 1 error): unused vars (150+), no-console (30+), no-non-null-assertion (20+), max-depth (1 error).

### Research Citations
- [ESLint v9 Migration Guide](https://eslint.org/docs/latest/use/migrate-to-9.0.0) - Latest version best practices
- [TypeScript ESLint Best Practices](https://typescript-eslint.io/linting/troubleshooting/performance-troubleshooting) - Performance optimization

### Atomic Subtasks

| # | Task | Files (≤5) | Est. |
|---|------|------------|------|
| 3.1 | Fix max-depth error in tests | `tests/tasks-destructuring.spec.ts` | 15min |
| 3.2 | Remove unused imports (30 files) | Various files | 45min |
| 3.3 | Prefix unused vars with underscore | Various files | 30min |
| 3.4 | Replace console.log with logger | Various files | 60min |
| 3.5 | Replace non-null assertions | Various files | 45min |
| 3.6 | Fix unused parameters (MarkdownComponents) | `src/components/SpecOutput/MarkdownComponents.tsx` | 20min |
| 3.7 | Clean up test file unused vars | `tests/**/*.spec.ts` | 30min |
| 3.8 | Fix react-refresh violations | Various files | 20min |
| 3.9 | Run lint --fix where possible | Terminal | 5min |
| 3.10 | Verify 0 warnings/errors | Terminal | 5min |
| 3.11 | Update lint rules if needed | `eslint.config.js` | 15min |
| 3.12 | Add pre-commit hook | `.husky/pre-commit` | 15min |
| 3.13 | Document lint standards | `CLAUDE.md` | 10min |
| 3.14 | Run full validation | Terminal | 5min |
| 3.15 | Update CHANGELOG | `CHANGELOG.md` | 5min |

---

## Action 4: Edge Function TypeScript Strict Mode

**Confidence: 88%** | **Priority: HIGH** | **Effort: Medium**

### Problem
Edge Functions in `supabase/functions/` don't have strict TypeScript configuration. Only frontend has strict mode enabled.

### Research Citations
- [Deno TypeScript Configuration](https://docs.deno.com/runtime/fundamentals/typescript/) - Official Deno/TypeScript integration
- [Supabase Edge Functions Best Practices](https://supabase.com/docs/guides/functions/best-practices) - Official recommendations

### Atomic Subtasks

| # | Task | Files (≤5) | Est. |
|---|------|------------|------|
| 4.1 | Create strict deno.jsonc | `supabase/functions/deno.jsonc` | 15min |
| 4.2 | Enable strict mode | `supabase/functions/deno.jsonc` | 5min |
| 4.3 | Run typecheck on functions | Terminal | 10min |
| 4.4 | Fix multi-agent-spec types | `supabase/functions/multi-agent-spec/**/*.ts` | 45min |
| 4.5 | Fix voice-to-text types | `supabase/functions/voice-to-text/index.ts` | 20min |
| 4.6 | Fix lib/ utility types | `supabase/functions/lib/**/*.ts` | 30min |
| 4.7 | Fix tools/ types | `supabase/functions/tools/**/*.ts` | 25min |
| 4.8 | Replace non-null assertions | Various files | 30min |
| 4.9 | Add type guards where needed | Various files | 25min |
| 4.10 | Run typecheck again | Terminal | 5min |
| 4.11 | Update CI/CD to check functions | `.github/workflows/*.yml` | 15min |
| 4.12 | Document in CLAUDE.md | `CLAUDE.md` | 10min |
| 4.13 | Update CHANGELOG | `CHANGELOG.md` | 5min |
| 4.14 | Test edge functions locally | Terminal | 15min |
...
### Problem
No visibility into type coverage metrics. Unknown percentage of codebase properly typed.

### Research Citations
- [type-coverage npm package](https://www.npmjs.com/package/type-coverage) - TypeScript coverage reporting
- [TypeScript Code Quality Metrics](https://dev.to/noriste/type-coverage-for-typescript-why-how-39kb) - Best practices for type coverage

### Atomic Subtasks

| # | Task | Files (≤5) | Est. |
|---|------|------------|------|
| 5.1 | Install type-coverage | `package.json` | 5min |
| 5.2 | Add npm script for coverage | `package.json` | 5min |
| 5.3 | Run initial type coverage scan | Terminal | 5min |
| 5.4 | Document baseline coverage | `docs/TYPE_COVERAGE.md` | 15min |
| 5.5 | Set coverage target (95%+) | `package.json` | 5min |
| 5.6 | Identify files with low coverage | Terminal | 10min |
| 5.7 | Fix top 10 lowest coverage files | Various files | 90min |
| 5.8 | Add coverage check to CI | `.github/workflows/*.yml` | 15min |
| 5.9 | Create coverage badge | `README.md` | 10min |
| 5.10 | Document in CLAUDE.md | `CLAUDE.md` | 10min |
| 5.11 | Add to pre-commit hook | `.husky/pre-commit` | 10min |
| 5.12 | Update CHANGELOG | `CHANGELOG.md` | 5min |
| 5.13 | Set up coverage trending | CI/CD config | 20min |

---

## Action 6: Pre-Commit Hook for Type Safety

**Confidence: 90%** | **Priority: HIGH** | **Effort: Low**

### Problem
No pre-commit hooks to prevent type errors and lint violations. Developers can commit broken code.

### Research Citations
- [Husky v9 Documentation](https://typicode.github.io/husky/) - Modern git hooks
- [lint-staged Best Practices](https://github.com/lint-staged/lint-staged) - Staged file linting

### Atomic Subtasks

| # | Task | Files (≤5) | Est. |
|---|------|------------|------|
| 6.1 | Install husky v9 | `package.json` | 5min |
| 6.2 | Install lint-staged | `package.json` | 5min |
| 6.3 | Initialize husky | Terminal | 5min |
| 6.4 | Create pre-commit hook | `.husky/pre-commit` | 10min |
| 6.5 | Configure lint-staged | `package.json` or `.lintstagedrc` | 15min |
| 6.6 | Add typecheck to staged files | Configuration | 10min |
| 6.7 | Add ESLint to staged files | Configuration | 5min |
| 6.8 | Test hook with staged changes | Terminal | 10min |
| 6.9 | Add commit-msg hook (optional) | `.husky/commit-msg` | 10min |
| 6.10 | Document hook workflow | `CLAUDE.md` | 10min |
| 6.11 | Update CHANGELOG | `CHANGELOG.md` | 5min |

---

## Action 7: TypeScript Project References

**Confidence: 82%** | **Priority: MEDIUM** | **Effort: High**

### Problem
Single monolithic TypeScript project. Slow incremental builds. No separation between app, tests, and edge functions.

### Research Citations
- [TypeScript Project References](https://www.typescriptlang.org/docs/handbook/project-references.html) - Official documentation
- [Faster TypeScript Builds](https://turborepo.org/blog/you-might-not-need-typescript-project-references) - Tradeoffs analysis

### Atomic Subtasks

| # | Task | Files (≤5) | Est. |
|---|------|------------|------|
| 7.1 | Create base tsconfig.base.json | `tsconfig.base.json` | 20min |
| 7.2 | Configure app project reference | `tsconfig.app.json` | 15min |
| 7.3 | Configure test project reference | `tsconfig.test.json` | 15min |
| 7.4 | Configure node project reference | `tsconfig.node.json` | 10min |
| 7.5 | Update root tsconfig.json | `tsconfig.json` | 10min |
| 7.6 | Add composite flag to sub-projects | Various tsconfig files | 10min |
| 7.7 | Enable incremental builds | Various tsconfig files | 5min |
| 7.8 | Test build performance | Terminal | 15min |
| 7.9 | Update build scripts | `package.json` | 10min |
| 7.10 | Verify IDE integration | VS Code | 10min |
| 7.11 | Add to CI/CD | `.github/workflows/*.yml` | 15min |
| 7.12 | Document structure | `CLAUDE.md` | 15min |
| 7.13 | Measure before/after build times | Terminal | 10min |
| 7.14 | Update CHANGELOG | `CHANGELOG.md` | 5min |
| 7.15 | Add troubleshooting guide | `docs/TYPESCRIPT_REFERENCES.md` | 20min |
| 7.16 | Test in production build | Terminal | 10min |

---

## Action 8: Additional Strict Flags (noPropertyAccessFromIndexSignature)

**Confidence: 87%** | **Priority: MEDIUM** | **Effort: Medium**

### Problem
Index signatures allow property access without runtime checks. `obj.foo` instead of `obj["foo"]` for index signatures.

### Research Citations
- [TypeScript 4.2 Release](https://devblogs.microsoft.com/typescript/announcing-typescript-4-2/) - Flag introduction
- [TypeScript Strict Property Access](https://mariusschulz.com/blog/strict-property-initialization-in-typescript) - Best practices

### Atomic Subtasks

| # | Task | Files (≤5) | Est. |
|---|------|------------|------|
| 8.1 | Enable noPropertyAccessFromIndexSignature | `tsconfig.app.json` | 5min |
| 8.2 | Run typecheck to find violations | Terminal | 5min |
| 8.3 | Fix violations in hooks/ | `src/hooks/**/*.ts` | 30min |
| 8.4 | Fix violations in components/ | `src/components/**/*.tsx` | 40min |
| 8.5 | Fix violations in lib/ | `src/lib/**/*.ts` | 25min |
| 8.6 | Add bracket notation where needed | Various files | 20min |
| 8.7 | Run typecheck again | Terminal | 5min |
| 8.8 | Run tests | Terminal | 10min |
| 8.9 | Document in CHANGELOG | `CHANGELOG.md` | 5min |
| 8.10 | Update CLAUDE.md | `CLAUDE.md` | 5min |
...
### Problem
Edge function utilities have tests, but no integration tests for full 8-stage pipeline or E2E request/response validation.

### Research Citations
- [Deno Testing Guide](https://docs.deno.com/runtime/fundamentals/testing/) - Official Deno testing
- [Supabase Edge Functions Testing](https://supabase.com/docs/guides/functions/unit-test) - Official recommendations

### Atomic Subtasks

| # | Task | Files (≤5) | Est. |
|---|------|------------|------|
| 9.1 | Create integration test setup | `supabase/functions/tests/integration-setup.ts` | 30min |
| 9.2 | Mock external API responses | `supabase/functions/tests/mocks/apis.ts` | 30min |
| 9.3 | Test multi-agent-spec E2E | `supabase/functions/tests/multi-agent-spec-e2e.test.ts` | 60min |
| 9.4 | Test stage progression | `supabase/functions/tests/multi-agent-spec-e2e.test.ts` | 30min |
| 9.5 | Test error handling | `supabase/functions/tests/multi-agent-spec-e2e.test.ts` | 30min |
| 9.6 | Test rate limiting | `supabase/functions/tests/rate-limiting.test.ts` | 30min |
| 9.7 | Test voice-to-text E2E | `supabase/functions/tests/voice-to-text-e2e.test.ts` | 30min |
| 9.8 | Test auth validation | `supabase/functions/tests/auth.test.ts` | 25min |
| 9.9 | Add test coverage reporting | `deno.jsonc` | 15min |
| 9.10 | Add to CI/CD pipeline | `.github/workflows/*.yml` | 20min |
| 9.11 | Document testing strategy | `docs/EDGE_FUNCTION_TESTING.md` | 20min |
| 9.12 | Create test data fixtures | `supabase/functions/tests/fixtures/*.ts` | 30min |
| 9.13 | Test timeout scenarios | `supabase/functions/tests/timeouts.test.ts` | 25min |
| 9.14 | Test concurrent requests | `supabase/functions/tests/concurrency.test.ts` | 30min |
| 9.15 | Update CHANGELOG | `CHANGELOG.md` | 5min |
| 9.16 | Add performance benchmarks | `supabase/functions/tests/benchmarks.test.ts` | 30min |
...
### Problem
Environment variables accessed via `import.meta.env.VITE_*` without type safety. Missing `.env.example`.

### Research Citations
- [Vite Environment Variables](https://vite.dev/guide/env-and-mode) - Official Vite documentation
- [Type-Safe Environment Variables with Zod](https://www.totaltypescript.com/validate-environment-variables-with-zod) - Best practices

### Atomic Subtasks

| # | Task | Files (≤5) | Est. |
|---|------|------------|------|
| 10.1 | Create .env.example | `.env.example` | 10min |
| 10.2 | Create env.d.ts with types | `src/env.d.ts` | 20min |
| 10.3 | Create Zod schema for env vars | `src/lib/env-validation.ts` | 25min |
| 10.4 | Add env validation to app init | `src/main.tsx` | 15min |
| 10.5 | Create helpful error messages | `src/lib/env-validation.ts` | 15min |
| 10.6 | Update all env var access | `src/**/*.ts{x}` | 30min |
| 10.7 | Add env var documentation | `CLAUDE.md`, `README.md` | 15min |
| 10.8 | Test with missing vars | Terminal | 10min |
| 10.9 | Test with invalid vars | Terminal | 10min |
| 10.10 | Add to pre-commit check | `.husky/pre-commit` | 10min |
| 10.11 | Document in CHANGELOG | `CHANGELOG.md` | 5min |
| 10.12 | Create setup guide | `docs/ENVIRONMENT_SETUP.md` | 20min |

---

## Implementation Timeline

### Week 1 (Critical Type Safety Foundation)
- [ ] Action 1: Consolidate to "strict": true Mode
- [ ] Action 2: Enable noUncheckedIndexedAccess
- [ ] Action 6: Pre-Commit Hook for Type Safety
...
### Week 3 (Advanced Type Safety & Monitoring)
- [ ] Action 5: Type Coverage Reporting & Monitoring
...
| CLAUDE.md | Dec 26, 2025 | ⚠️ Needs TypeScript strict mode update |
...
- [ESLint v9 Docs](https://eslint.org/docs/latest/) - ✅ Current (2025)
- [Vite 7 Docs](https://vite.dev/) - ✅ Current (2025)
- [Husky v9 Docs](https://typicode.github.io/husky/) - ✅ Current (2025)
- [type-coverage](https://www.npmjs.com/package/type-coverage) - ✅ Current (v2.x, 2025)
- [lint-staged](https://github.com/lint-staged/lint-staged) - ✅ Current (v15.x, 2025)

---

# Previous Completed Actions (Phases 1-6)
